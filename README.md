# team-whimsy
Repository for games made for NCSU CSC486

Group: Danielle Brown, Ethan Flood, Dominic Latiolais, Hope Spence
## Puzzle Game: Mind's Molding - README
### Overview
This beta includes the full functionality of one of our core puzzles.

### Controls:
- Arrow keys: movement
- Mouse/Mousepad (drag and drop): puzzle interaction

### Special Instructions:
You can explore the environment, but you should run into the magic circle to activate its puzzle. 

### Sources
NPC Dialogue by Game Code Library: https://www.youtube.com/watch?v=eSH9mzcMRqw 

Top Down Movement by Game Code Library: https://www.youtube.com/watch?v=DQY62meLVCk&list=PLaaFfzxy_80HtVvBnpK_IjSC8_Y9AOhuP&index=3&t=309s 

Pixel Font Asset by Craftron Gaming

Interaction System by Game Code Library: https://www.youtube.com/watch?v=MPP9GLp44Pc

Free sound effects: https://kenney.nl/assets

Unity Documentation:
Unity Mouse Input: https://docs.unity3d.com/Packages/com.unity.inputsystem@0.2/manual/Mouse.html 
Unity Lerp Documentation: https://docs.unity3d.com/6000.3/Documentation/ScriptReference/Vector2.Lerp.html 

Mouse Inputs & Mouse Position Tutorial by Modding by Kaupenjoe: https://www.youtube.com/watch?v=zfUCf7_-dMs 

## Dungeon Crawler: Moth's Ascent
### Overview
Moth's Ascent is a turn-based 2d game in which the player must fight enemies that will drop items and allow the player to metamorphosize, giving them new abilities to use in future fights. Following the previous game Worm's Wake, our player awakes within The Forest of Whim and discovers that the creatures within have been taken over by a fungus. These now infected plants and creatures stand in the way of you discovering the source, and the player now needs to fight their way to the end, in which they must face the final boss that guards the path to the infection's heart.

Special Commands: 

Press ‘y’ to open and close inventory

Press esc to open and close pause menu

### Sources
General C#

C# Succinctly by Joe Mayo
https://www.syncfusion.com/succinctly-free-ebooks/csharp

Documentation in C# by Ravi Vishwakarma
https://dev.to/ravivis13370227/comprehensive-guide-to-documentation-comments-in-c-379d

Game Development in C#

Unity Prototype 3D Turn-Based Game Repository by i-Jiro
https://github.com/i-Jiro/Unity3D-Turn_Based_RPG/tree/main

Turn-Based Battle System by Brackeys
https://www.youtube.com/watch?v=_1pz_ohupPs

2D Top-Down Tutorial Series by Game Code Library
https://www.youtube.com/playlist?list=PLaaFfzxy_80HtVvBnpK_IjSC8_Y9AOhuP

Procedural Room Generation by Himsl Games
https://www.youtube.com/watch?v=1-HIA6-LBJc

Unity Documentation
https://docs.unity3d.com/6000.3/Documentation/Manual/index.html

Introduction to Unity Unit Testing by Ben MacKinnon
https://www.kodeco.com/38240193-introduction-to-unity-unit-testing

Inventory Building in Unity by Night Run Studio
https://www.youtube.com/watch?v=LaQp5u0_UYk

START MENU in Unity by Brackeys
https://www.youtube.com/watch?v=zc8ac_qUXQY

SETTINGS MENU in Unity by Brackeys
https://www.youtube.com/watch?v=YOaYQrN1oYQ

Pixel Art Assets

32x32 Forest Tileset Pack by ToffeeCraft
https://toffeecraft.itch.io/forest-nature-pack

32x32 Fantasy RPG by Mixel
https://mixelslime.itch.io/free-top-down-rpg-32x32-tile-set

Asset Packs by Kenney
https://kenney.nl/assets

Pixel Font Asset by Craftron Gaming
https://www.dafont.com/minecraft.font

RPG World Caves by Szadi Art.
https://assetstore.unity.com/packages/2d/environments/rpg-worlds-caves-167274

Ninja Adventure Asset Pack by Pixel-Boy and AAA
https://pixel-boy.itch.io/ninja-adventure-asset-pack

Pixel Lock Icon created by menon - Flaticon
https://www.flaticon.com/free-icons/pixe

Music and SFX Assets

https://freemusicarchive.org/home

https://kenney.nl/assets



